...
//        Sequence<Integer> sCopy = new Sequence1L<Integer>();
//        s.extract(0, s.length() - 1, sCopy);
        boolean result = true;
        if (s.length() > 1) {
            while (s.length() >= 1 && !result) {
                Integer first = s.entry(0);
                Integer last = s.entry(s.length() - 1);
